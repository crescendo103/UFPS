...
AAIEnemy::AAIEnemy()
{
    ...
    BoxCollision->OnComponentBeginOverlap.AddDynamic(
        this, &AAIEnemy::OnBoxOverlap
    );
    BoxCollision->OnComponentEndOverlap.AddDynamic(
        this, &AAIEnemy::OnBoxEndOverlap
    );
    ...
    CapsuleCollision->OnComponentBeginOverlap.AddDynamic(
        this, &AAIEnemy::OnCapsuleOverlap
    );
...
void AAIEnemy::OnCapsuleOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
    ...
    // 🔥 Weapon의 TriggerSphere만 허용
    if (!OtherComp->IsA<USphereComponent>())
        return;
    */
    AActor* Actor = OtherActor;

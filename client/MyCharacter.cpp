...
void AMyCharacter::CameraLineTrace()
{
    ...
    if (bHit && Cast<AWeaponActor>(Hit.GetActor()))
    {
...
void AMyCharacter::StartFire()
{
    ...
    if (CurrentGunWeapon)
    {
        ...
        // 카메라 위치에서 방향으로 일정 거리만큼 앞으로 이동한 지점을 시작점으로 사용
        const float MuzzleForwardOffset = 100.0f; // 원하는 거리(cm)로 조절
        FVector SpawnLocation = CameraLocation + WorldDir * MuzzleForwardOffset;

        // 시작지점 = 카메라 위치, 방향 = 화면 중앙
        CurrentGunWeapon->SpawnItem(SpawnLocation, WorldDir);
    }
...
void AMyCharacter::ChangeFirstWeapon()
{
    ...
    if (InventoryWidget)
    {
        if (AActor* SecondWeaponVisualActor = InventoryWidget->SecondWeaponVisualActor)
        {
            SecondWeaponVisualActor->SetActorHiddenInGame(true);
            SecondWeaponVisualActor->SetActorEnableCollision(false);
        }
    }   
    isSubWeaponActive = false;     
}
...
void AMyCharacter::ChangeSecondWeapon()
{
    ...
    if (InventoryWidget)
    {
        if (AActor* SecondWeaponVisualActor = InventoryWidget->SecondWeaponVisualActor)
        {
            SecondWeaponVisualActor->SetActorHiddenInGame(false);
            SecondWeaponVisualActor->SetActorEnableCollision(true);
        }
    }   
...
{    
    if (OtherActor && OtherActor->IsA(AWeaponActor::StaticClass()))
    {
        ...
        if (item)
        {
            // 이미 인벤토리 슬롯이나 무기칸에 등록된 액터라면 바닥 UI에 다시 띄우지 않음
            if (InventoryWidget->GetItemIndexFromActor(item) != INDEX_NONE)
                return;

            if (InventoryWidget->SecondWeaponVisualActor == item)
                return;

...
void GameSession::StartSession()
{
    ...
    // ───── 블루홀: 시작 즉시 1회 ─────
    {
        Vector3 BluePos = PosMgr.GetRandomMapPoint();

        FSpawnAIPacket BlueHolePacket{};
        BlueHolePacket.Header.Size = sizeof(FSpawnAIPacket);
        BlueHolePacket.Header.Type = static_cast<int>(EPacketType::BlueHole);
        BlueHolePacket.X = BluePos.x;
        BlueHolePacket.Y = BluePos.y;
        BlueHolePacket.Z = BluePos.z;

        Broadcast((char*)&BlueHolePacket, sizeof(BlueHolePacket));
        printf("[Session %d] BlueHole 발송 Pos=(%.1f, %.1f, %.1f)\n",
            SessionId, BluePos.x, BluePos.y, BluePos.z);
    }

    // ───── 레드존: N초 간격으로 총 2회 ─────
    std::thread([this]()
        {
            const int RedzoneCount = 2;
            const int IntervalSeconds = 60; // 원하는 간격으로 조정

            for (int i = 0; i < RedzoneCount; i++)
            {
                std::this_thread::sleep_for(std::chrono::seconds(IntervalSeconds));                

                Vector3 RedPos = PosMgr.GetRandomMapPoint();

                FSpawnAIPacket RedzonePacket{};
                RedzonePacket.Header.Size = sizeof(FSpawnAIPacket);
                RedzonePacket.Header.Type = static_cast<int>(EPacketType::Redzone);
                RedzonePacket.X = RedPos.x;
                RedzonePacket.Y = RedPos.y;
                RedzonePacket.Z = RedPos.z;

                Broadcast((char*)&RedzonePacket, sizeof(RedzonePacket));
                printf("[Session %d] Redzone 발송 (%d/%d) Pos=(%.1f, %.1f, %.1f)\n",
                    SessionId, i + 1, RedzoneCount, RedPos.x, RedPos.y, RedPos.z);
            }
        }).detach();
...
void GameSession::DataProcess(FPacketHeader* Header, SOCKET soc)
{
    ...
    case EPacketType::Character:
    {
        ...
        if (Char->CharacterId > -100)//플레이어 위치 동기화
        {

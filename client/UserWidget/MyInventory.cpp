...
//아이템이 플레이어에 주변에 있어 UI에 보이도록 하는 함수
...
//총기 스코프 같은거랑 연계해서 사용하려고 했는데 미완성
...
{   
    if (AWeaponActor* setItem = Cast<AWeaponActor>(actor)) {
        ...
        {
            SocketTransform = Owner->GetMesh()->GetSocketTransform(TEXT("RightHandPinky4Socket"), RTS_World);
        }
